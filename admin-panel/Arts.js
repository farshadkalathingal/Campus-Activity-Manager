...
$(document).ready(function(){
		 ...
		 for (var i in json_obj) 
			{
				 ...
				 $('select[name="selectEventResult"]').append('<option value="'+json_obj[i].Event_Name+'">' +json_obj[i].Event_Name+ "</option>");
			}
	  
	}
	});
	$("#selectEventResult").on('select',function(e){
		var event_id = $( "#selectEventResult" ).val();

		$.ajax({
			type: "POST",
			data: {
				
				 'action': 'getResultDetails',
				'Name': event_id      
				},
			url: "mainpro_Service_call.php",
			success:function(data){
	...
	$("#modifyResult").on('submit',function(e){
		var event_name = $( "#selectEventResult" ).val();
		var first = $( "#first" ).val();
		var second = $( "#second" ).val();
		var third = $( "#third" ).val();
		//alert(event_id);
